...
class Reader {
 ...
 public:
  ...
  SGOffset PeekNumNodes() {
    DirectFile file(filename_);
    bool directed;
    SGOffset num_edges, num_nodes;
    file.read_val(directed);
    file.read_val(num_edges);
    file.read_val(num_nodes);
    return num_nodes;
  }
  ...
  CSRGraph<NodeID_, DestID_, invert> ReadSerializedGraph() {
    ...
    file.read_exact(offsets.data(), num_index_bytes);
    file.read_exact(neighs, num_neigh_bytes);
    index = CSRGraph<NodeID_, DestID_>::GenIndex(offsets, neighs);
    if (directed && invert) {
      inv_neighs = new DestID_[num_edges];
      file.read_exact(offsets.data(), num_index_bytes);
      file.read_exact(inv_neighs, num_neigh_bytes);
