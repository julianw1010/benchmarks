...
pvector<ScoreT>& Brandes(const Graph &g, SourcePicker<Graph> &sp,
                         NodeID num_iters, pvector<ScoreT> &scores,
...
int main(int argc, char* argv[]) {
  ...
  Builder b(cli);
  int64_t num_nodes = b.PeekNumNodes();
  pvector<ScoreT> scores(num_nodes);
  pvector<CountT> path_counts(num_nodes);
  pvector<ScoreT> deltas(num_nodes);
  #pragma omp parallel for
  ...
  auto BCBound = [&sp, &cli, &scores, &path_counts, &deltas] (const Graph &g) -> pvector<ScoreT>& {
    return Brandes(g, sp, cli.num_iters(), scores, path_counts, deltas,
                   cli.logging_en());
  };
