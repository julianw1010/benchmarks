...
pvector<NodeID>& ShiloachVishkin(const Graph &g, pvector<NodeID> &comp) {
...
int main(int argc, char* argv[]) {
  ...
  for (int64_t n = 0; n < num_nodes; n++)
    comp[n] = 0;
  Graph g = b.MakeGraph();
  auto SVBound = [&comp](const Graph &g) -> pvector<NodeID>& {
    return ShiloachVishkin(g, comp);
  };
  BenchmarkKernel(cli, g, SVBound, PrintCompStats, CCVerifier);
  return 0;
}
