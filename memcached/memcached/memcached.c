...
static void preload_items(void) {
    uint64_t n = settings.preload_count;
    if (n == 0) return;

    const int data_size = 24;
    const int nbytes = data_size + 2;
    char data[26];
    ...
    uint64_t i;
    for (i = 1; i <= n; i++) {
        int nkey = snprintf(key, sizeof(key), "%llu", (unsigned long long)i);
        uint32_t hv = hash(key, nkey);
        item *it = do_item_alloc(key, nkey, 0, 0, nbytes);
        if (it == NULL) {
            fprintf(stderr, "Preload OOM at item %llu\n", (unsigned long long)i);
            break;
        }
        memcpy(ITEM_data(it), data, nbytes);
        do_item_link(it, hv, 0);
        do_item_remove(it);

        if (i % 1000000 == 0) {
            fprintf(stderr, "Preloaded %llu / %llu\n", (unsigned long long)i, (unsigned long long)n);
        }
    }
    fprintf(stderr, "Preload done: %llu items\n", (unsigned long long)(i - 1));
}
...
int main (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv, shortopts))) {
	   ...
	   case PRELOAD:
                if (subopts_value == NULL) {
                    fprintf(stderr, "Missing preload argument\n");
                    goto error;
                }
                if (!safe_strtoull(subopts_value, &settings.preload_count)) {
                    fprintf(stderr, "could not parse argument to preload\n");
                    goto error;
                }
                break;
#ifdef PROXY
